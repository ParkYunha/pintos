...
start_process(void *cmd)
{
  ...
  /*page_inti은 여기가 맞을까 thread_init이 맞을까 */
  page_init(&thread_current()->page_table);
...
bool load(const char *file_name, void (**eip)(void), void **esp)
{
  ...
  sema_up(&file_sema);
  if (file_r != sizeof ehdr || memcmp(ehdr.e_ident, "\177ELF\1\1\1", 7) || ehdr.e_type != 2 || ehdr.e_machine != 3 || ehdr.e_version != 1 || ehdr.e_phentsize != sizeof(struct Elf32_Phdr) || ehdr.e_phnum > 1024)
  {
             ...
             uint32_t read_bytes, uint32_t zero_bytes, bool writable)
{
  ...
  while (read_bytes > 0 || zero_bytes > 0)
  {
    ...
    uint8_t *kpage = allocate_page(upage, file, ofs, page_read_bytes, page_zero_bytes, writable);
    if (kpage == NULL)
      return false;
...
setup_stack(void **esp)
{
  ...
  // kpage = allocate_frame(*esp); //pj3 before stack growth

  // success = install_page (((uint8_t *) PHYS_BASE) - PGSIZE, kpage, true);
  // Allocate and initialize the first stack page. //
